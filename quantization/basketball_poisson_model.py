...
class BasPoissonModel( object ):
    ...
    def log_likelihood_category(self, ds='test' ):
        ...
        for k, v in dataset.items():
            llk = 0.
            count = 0
            lambda_ = self.params[k][0]
            # std = self.params[k][1]

            for vv in v:
                # iterate every item in value
                llk += np.log( poisson.pmf( vv, lambda_ ) )
                count += 1
...
if __name__=='__main__':
    ...
    # rrr = np.arange( -20,20,1 )
    # a = [ poisson.pmf( i , 10 ) for i in rrr ]
    # b = [ norm.pdf( i , loc=10, scale=np.sqrt(10) ) for i in rrr ]
    # fig, ax = plt.subplots()
    # ax.plot( rrr , a )
    # ax.plot( rrr, b )
    # plt.show()