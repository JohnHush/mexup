...
class BasBinomialModel( object ):
    ...
    def paramsEvaluation(self):
        ...
        for k, v in self.train_data.items():
            # use 1st order momentum to evaluate the parameter
            mean_evaluated = np.mean( v )
            p_evaluated = mean_evaluated / self._N

            self.params[k] = [ p_evaluated, ]
    ...
    def log_likelihood_category(self, ds='test' ):
        ...
        for k, v in dataset.items():
            ...
            for vv in v:
                ...
                llk += np.log( binom.pmf( vv, self._N, p_ ) )
                count += 1
