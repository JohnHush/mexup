...
class infer_soccer_model_input(object):
    ...
    def set_value(self, score, asian_handicap_market, over_under_market, max_total_goals, parameter, eps):
        self.score = score
    ...
    def infer_supremacy_total_goals(self):
        # 反查total goals
        ...
        #反查supremacy
        exp_supremacy_left = -exp_total_goals_middle+self.eps
        exp_supremacy_right = exp_total_goals_middle-self.eps
        ...
        while abs(self.asian_handicap_cost_function(exp_supremacy_middle, exp_total_goals_middle)) > self.eps:
            exp_supremacy_middle = (exp_supremacy_left+exp_supremacy_right)/2

            if self.asian_handicap_cost_function(exp_supremacy_left,exp_total_goals_middle)*self.asian_handicap_cost_function(exp_supremacy_middle, exp_total_goals_middle) <= 0:
                exp_supremacy_right = exp_supremacy_middle
            else:
                exp_supremacy_left = exp_supremacy_middle
            count_sup = count_sup+1
        return round(exp_supremacy_middle, 3), round(exp_total_goals_middle, 3)
