...
#更新对象参数
    def set_value(self, mu,score,parameter):
        ...
        self.goal_diff=self.score[0]-self.score[1]
        self.goal_sum=self.score[0]+self.score[1]
        self.prob_matrix=np.zeros([16,16])
        self.adj_mode=self.parameter[0]
        ...
        if self.adj_mode==0:
            self.draw_adj=self.parameter[1][0]
            self.draw_split=self.parameter[1][1]
...
# examp.double_chance_over_under(2.5)
# examp.home_over_under(0.75)
# examp.away_over_under(1.5)
# examp.home_exact_totals(3)
# examp.away_exact_totals(0)
# examp.home_winning_by(2)
# examp.away_winning_by(1)
# examp.both_scored()
# examp.odd_even()
